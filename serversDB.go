...
type Server struct {
	...
	IPType       int                 `json:"ip_typo" db:"ip_type"`         //动态，静态，是否经过nat
...
func addServers(s *Server) error {
	...
	//	fmt.Println("user:", e)
	query := `INSERT INTO servers (name,cpu_type,mem_size,input_rate,output_rate,netcard,
...
func updateServer(s *Server) error {
	...
	ip_type=$7,ip_addr=$8,dns_name=$9,server_type=$10,group_list=$11,ower_id=$12,ower_callsign=$13,status=$14,note=$15, update_time=now() where id=$16`,
		s.Name, s.CpuType, s.MemSize, s.InputRate, s.OuputRate, s.NetCard,
		s.IPType, s.IPAddr, s.DNSName, s.ServerType, s.GroupList, s.OwerID, s.OwerCallsign, s.Status, s.Note, s.ID)
	if err != nil {
